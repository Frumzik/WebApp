...
function sendAnswer(event) {
    ...
    .then(response => {
        ...
        if (response.status === 204) {
            console.log('Ответ сервера: пустой (No Content)');
            showToast('answerSent'); 
            return; 
        }
        if (!response.ok) {
            return response.json().then(error => {
                console.error('Ошибка отправки:', error);
                throw new Error(`Ошибка: ${error.message || 'Неизвестная ошибка'}`);
            });
        }
    ...
    fetch(`/api/series/play/?series_id=${seriesId}&${params}`, {
        ...
        .then(data => {
            ...
            pages.forEach((page, index) => {
                ...
                if (index === 0) {
                    slideHTML = `
                        <div class='icon-container' style="position: absolute; bottom: 100px; right: 50px; z-index: 100;">
                            <svg width="70" height="70" viewBox="0 0 62 62" fill="none" xmlns="http://www.w3.org/2000/svg">
